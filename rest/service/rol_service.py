...
class RolService:
    ...
    async def update_rol(db: AsyncSession, rol_id: uuid.UUID, rol_update: RolUpdate) -> Tuple[Optional[Rol], Optional[str]]:
        ...
        if rol_update.categoria is not None or rol_update.subcategoria is not None:
            if rol_update.categoria != existing_rol.categoria or rol_update.subcategoria != existing_rol.subcategoria:
                return None, "No se puede modificar categoría o subcategoría. Elimine el rol y cree uno nuevo"
    ...
    async def delete_rol(db: AsyncSession, rol_id: uuid.UUID) -> Tuple[bool, Optional[str]]:
        ...
        active_users = await UsuarioDAO.count_active_by_rol(db, rol_id)
        if active_users > 0:
            return False, f"No se puede eliminar el rol. Tiene {active_users} usuario(s) activo(s) asociado(s)"
