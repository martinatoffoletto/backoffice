...
async def get_all_clases(
    ...
    status_filter: Optional[bool] = Query(None, alias="status", description="Filtrar por estado activo/inactivo"),
    ...
    try:
        ...
        if param is not None and value is not None:
            ...
            return await ClaseIndividualService.search_clases(db, param, value, skip, limit, status_filter)
        else:
            # Si no hay parámetros de búsqueda, devolver todas las clases
            return await ClaseIndividualService.get_all_clases(db, skip, limit, status_filter)
...
async def get_clases_by_curso(
    ...
    """
    try:
        return await ClaseIndividualService.get_clases_by_curso(db, id_curso, skip, limit, status_filter)
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def get_clases_by_estado(
    ...
    """
    try:
        return await ClaseIndividualService.get_clases_by_estado(db, estado, skip, limit, status_filter)
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def get_clases_by_fecha(
    ...
    """
    try:
        return await ClaseIndividualService.get_clases_by_fecha(db, fecha, skip, limit, status_filter)
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def get_clases_by_fecha_range(
    ...
    """
    try:
        return await ClaseIndividualService.get_clases_by_fecha_range(db, fecha_inicio, fecha_fin, skip, limit, status_filter)
    except ValueError as e:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=str(e)
...
async def get_clase_by_id(
    ...
    """
    try:
        clase = await ClaseIndividualService.get_clase_by_id(db, id_clase, status_filter)
        if not clase:
            raise HTTPException(
