...
@router.post("/{id_rol}/restore")
async def restore_rol(
    id_rol: int,
    updated_by: str = Query(..., description="Usuario que restaura el rol"),
    db: Session = Depends(get_db)
):
    """
    ...
    try:
        rol_service = RolService(db)
        result = rol_service.restore_rol(id_rol, updated_by)
        return result
...
async def get_rol_statistics(db: Session = Depends(get_db)):
    """
    Obtener estadísticas de roles.
    """
    try:
        rol_service = RolService(db)
        result = rol_service.get_rol_statistics()
        return result
