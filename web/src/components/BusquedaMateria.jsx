...
import { carreraPorId, carreraPorNombre, verMateriasPorCarrera } from "@/api/carrerasApi";
...
export default function BusquedaMateria({onMateriaSeleccionada}) {
    ...
    const [showDropdownCarrera, setShowDropdownCarrera] = useState(false);
    const [suggestions, setSuggestions]=useState([]);
    const [carreraSuggestions, setCarreraSuggestions] = useState([]);
        ...
        <div className="w-full flex flex-col items-center">
                ...
                <div className="flex flex-col items-center lg:flex-row lg:items-center justify-between my-4 gap-4">
                    ...
                    <div className="relative w-full max-w-6xl">
                        <Input
                            ...
                            value={carreraName}
                            onChange={(e) =>{
                                const textoC=e.target.value;
                                setCarreraName(textoC);

                                if(textoC.trim()===""){
                                    setCarreraSuggestions([]);
                                    setShowDropdownCarrera(false);
                                    return;
                                }

                                const suger_Carreras= resultados_state.filter((carrera)=> 
                                   carrera.nombre.toLowerCase().includes(textoC.toLowerCase())
                                )
                                setCarreraSuggestions(suger_Carreras);
                                setShowDropdownCarrera(true);
                            }}
                        />
                        {showDropdownCarrera && carreraSuggestions.length > 0 && (
                            <Command className="absolute left-0 right-0 bg-white border rounded-md shadow-md mt-1 z-50 min-h-fit max-h-60 overflow-y-auto">
                                <CommandGroup heading="Coincidencias">
                                    {carreraSuggestions.map((carrera) => (
                                    <CommandItem
                ...
                <div className="overflow-x-auto mt-8">
                        ...
                        <TableBody>
                            {paginatedMaterias.map((materia) => (
                                ...
                                >
                                    ...
                                    <TableCell className={materia.status === "activo" ? "text-green-600" : "text-red-600"}>
                                        {materia.status || "-"}
                                    </TableCell>
                                    <TableCell>
                                        <Button 
                    ...
                    {totalPages > 1 ? (
                        ...
                        <p className="text-center text-gray-500 text-sm mt-4">
                            {resultados_state.length} resultados encontrados
                        </p>
                    )}
                </div>
            )}
            ...
            {showOpciones && materiaSeleccionada && (
                ...
                <div className="bg-white p-6 rounded-lg shadow-xl border-2 border-blue-500 w-96 max-w-md">
                    <h2 className="text-xl font-bold mb-4 ">Opciones para la materia</h2>
