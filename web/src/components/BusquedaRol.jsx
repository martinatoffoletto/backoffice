...
import { buscarRoles, obtenerRoles } from "@/api/rolesApi";

export default function BusquedaRol() {
  const [param, setParam] = useState("categoria");
  ...
  const handleSearch = async () => {
    ...
    setError(null);
    try {
      const response = await buscarRoles(param, value, status_filter);
      setResultados(response);
    } catch (err) {
      setError(err.response?.data?.detail || err.message || "Error al buscar roles");
      setResultados([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleLoadAll = async () => {
    ...
    setError(null);
    try {
      const response = await obtenerRoles(status_filter);
      setResultados(response);
    } catch (err) {
      setError(err.response?.data?.detail || err.message || "Error al cargar roles");
      setResultados([]);
    } finally {
      setLoading(false);
    }
              ...
              <TableBody>
                {resultados.map((rol) => (
                  <TableRow key={rol.id_rol}>
                    ...
                    <TableCell>${rol.sueldo_base || 0}</TableCell>
                    <TableCell>
                      {rol.status ? (
