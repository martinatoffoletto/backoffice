...
export default function BusquedaSede() {
  ...
  const handleSearch = async () => {
    ...
    setError(null);
    try {
      const response = await buscarSedes(param, value);
      setResultados(response);
    } catch (err) {
      setError(err.response?.data?.detail || err.message || "Error al buscar sedes");
      setResultados([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleLoadAll = async () => {
    ...
    setError(null);
    try {
      const response = await obtenerSedes(0, 100, status_filter);
      setResultados(response);
    } catch (err) {
      setError(err.response?.data?.detail || err.message || "Error al cargar sedes");
      setResultados([]);
    } finally {
      setLoading(false);
    }
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>Cantidad de Aulas</TableHead>
              ...
              <TableBody>
                {resultados.map((sede) => (
                  <TableRow key={sede.id_sede}>
                    ...
                    <TableCell>{sede.cantidad_aulas || "-"}</TableCell>
                    <TableCell>
                      {sede.status !== false ? (
